...
Every field in the schema must be present. If a field is genuinely absent, set `value` to null and report confidence as the literal number `0.00` (not `null`). (Use `null`, not `[]`, for an absent `schedules` value — downstream consumers treat "field absent" uniformly.)
