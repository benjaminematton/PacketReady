...
- `canonicalSpecialty` — the NUCC Display Name for a taxonomy code printed on the provider's state medical license (e.g. `"Cardiovascular Disease Physician"`).
- `statedSpecialty` — the specialty printed on the provider's board certification (e.g. `"Cardiology"`).
