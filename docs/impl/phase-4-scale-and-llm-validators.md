...
- [x] `evals/dataset/` holds **50 packets** in 4 buckets: 15 clean+valid · 15 clean+conflicts · 15 scanned+clean · 5 scanned+conflicts. Generated programmatically from NPPES distributions with a locked random seed; **deterministic field values; PDF bytes may vary across ReportLab versions and platform JPEG encoders**.
...
- [x] **Two new LLM-augmented validators wired:** `identity_coherence` (Sonnet structured output, `fullName` cross-doc agreement — DOB/NPI/address explicitly deferred to P4.5 per the validator's own XML doc; deliberately narrowed from the original four-field scope to keep one prompt under tuning at a time) and `npi_taxonomy_match` (CSV-deterministic lookup of taxonomy code → canonical specialty, then a thin Sonnet "does stated specialty match canonical?" call). Both registered in `AddApplication()` and exercised by `ComputeReadinessScoreCommand`.
- [x] **Three new payer-aware validators wired:** `malpractice_currency` (status + coverage ≥ payer minimum + ≥ 30-day window; owns the full malpractice surface — no other malpractice validator exists), `required_documents` (per-payer required-doc-type presence), and `BoardCertificationValidator` extended to consume payer config for the accepted-boards list. Three sample payers committed (`payer-a-national-hmo.yaml`, `payer-b-state-medicaid.yaml`, `payer-eval-seed.yaml`).
- [x] **Conflict precision + recall** reported per `plantedConflict` kind. Definition: a planted conflict is "caught" iff **all three** of: (1) at least one Issue's `Validator` matches the expected validator for that kind, (2) the Issue's citations name at least one of the planted `sources`, and (3) the Issue's `Field` (on LLM-emitted Issues) matches the planted conflict's target field. Precision = caught-and-planted / all-flagged-conflicts; recall = caught-and-planted / total-planted.
- [x] **Tier agreement on 20 hand-labeled packets** — headline metric is **weighted Cohen's κ (quadratic weights) ≥ 0.50** (substantial-agreement floor per Landis-Koch) with the **3×3 confusion matrix** and **raw agreement rate** reported alongside. Spearman is *not* the headline (3-tier categorical labels make it unstable at n=20); kept as a footnote for design-doc continuity. Labels live at `evals/labels/human_tiers.json`, written **before** the eval runs so the labeler isn't anchored on system output (enforced by an mtime check). **Locked in 2026-05-26**: κ = 0.6786, raw = 0.55, ρ = 0.7455, confusion = [[2,8,0],[0,2,0],[0,1,7]], n = 20.
- [x] **Confidence-threshold gate** active: a Critical Issue whose citations include any field with confidence < 0.85 is downgraded to Minor and carries a structural `isLowConfidenceInput: true` flag on the Issue itself (not a message suffix). Documented in [design.md §11.1](../design.md).
- [x] **`evals/results/baseline.json`** committed with the full-pipeline payload shape and real numbers — `successCount`, `conflicts` (per-kind P/R), `scoreResults` (50 rows), `agreement` (κ + 3×3 + ρ + biasNote). The P3 `stub` flag belonged to the field-accuracy runner's separate schema and doesn't carry into the score-side full-pipeline output; the regression gate now guards the full-pipeline shape in earnest.
- [x] **README.md** ships with the accuracy table, conflict precision/recall, Spearman correlation, the [design.md Appendix A](../design.md) competitor comparison row, and one paragraph naming the labeler-bias caveat in plain language. (Closed 2026-05-26 with the κ + 3×3 publish.)
- [x] `dotnet test` and the Python runner both pass — **577/577 C#, 162/162 Python green**. Per-LLM-validator test files for `IdentityCoherence` and `NpiTaxonomyMatch`; per-pure-code files for `MalpracticeCurrency`, `RequiredDocuments`, and the `BoardCertification` extension — exceeds the "one per LLM + one per pure-code" minimum.
...
- **NPPES sample staleness.** The synthetic snapshot was generated 2026-05-25 against US Census 2020 + AAMC 2022 distributions; specialty / state mixes don't shift fast but they do shift. Treat the file as immutable for P4; rerun `build_nppes_sample.py` in a P6 follow-on if licensure patterns visibly drift (and only then — the regression gate's stability depends on the file's md5 not moving between runs).
...
- **Clean buckets carry an incidental Red signal from DEA-anchor math.** [packets.py](../../evals/generators/packetready_eval/packets.py) sets `_NEW_PACKET_ANCHOR = date(2026, 5, 25)` and draws `dea_issue = anchor - 365 * rng.randint(1, 3)` → `dea_expiry = dea_issue + 365 * 3`. When `rng = 3`, `dea_expiry` lands exactly on the anchor date, so against any `today > 2026-05-25` those packets read as "DEA expired yesterday" → Red on expiry alone. Hits ~1/3 of clean/scanned packets (4 of 14 in the labeling subset: Hall, Flores, Rice, Tucker). **Not a bug — it gives the eval Red samples in the non-conflict buckets so κ has variance even before planted conflicts.** README's dataset description should name this so a reader doesn't expect all "clean" packets to score Green. If we ever regen, shift to `rng.randint(0, 2)` to make clean buckets strictly future-DEA.
